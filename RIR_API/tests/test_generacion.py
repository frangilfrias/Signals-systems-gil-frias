...
class TestGenerarSineSweep:
    ...
    def test_sine_sweep_rango_frecuencias(self):
        ...
        assert energia_f1 > 1e-10, (
            "No se detectó energía significativa cerca de la frecuencia inicial (20 Hz)"
        )
        assert energia_f2 > 1e-10, (
            "No se detectó energía significativa cerca de la frecuencia final (20000 Hz)"
        )
        ...
        assert porcentaje_creciente > 0.9, (
            "La frecuencia instantánea no presenta crecimiento monotónico suficiente"
        )
    ...
    def test_sine_sweep_convolucion_impulso(self):
        ...
        # Excluyo la ventana
        ventana = 100
        resto = np.concatenate([respuesta[: idx_pico - ventana], respuesta[idx_pico + ventana :]])

        # Mido la energia promedio del resto
        energia_promedio_resto = np.mean(np.abs(resto))

        # Comparo el pico con el resto
        relacion_db = 20 * np.log10(pico / energia_promedio_resto)
