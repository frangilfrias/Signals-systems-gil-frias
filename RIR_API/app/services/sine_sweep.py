...
def generar_sine_sweep(
    ...
    if not isinstance(f1, (int, float)):
        raise TypeError(f"f1 debe ser int o float. Tipo recibido: {type(f1).__name__}, valor: {f1}")

    if not isinstance(f2, (int, float)):
        raise TypeError(f"f2 debe ser int o float. Tipo recibido: {type(f2).__name__}, valor: {f2}")
    ...
    if f1 <= 0:
        raise ValueError("f1 debe ser mayor que 0 Hz.")

    if f2 <= 0:
        raise ValueError("f2 debe ser mayor que 0 Hz.")

    if f1 >= f2:
        raise ValueError("f1 debe ser menor que f2.")

    if duracion <= 0:
        raise ValueError("La duración debe ser mayor que 0 segundos.")

    if fs not in fs_validas:
        raise ValueError(f"fs inválida: {fs}. Valores permitidos: {fs_validas}")
    ...
    compensacion_temp = np.exp(
        -eje_tem / k_param
    )  # Compensa la distribución no uniforme de energía del sweep logarítmico
